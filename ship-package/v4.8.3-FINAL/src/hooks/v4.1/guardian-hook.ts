...
export function createGuardianHook(guardian: Guardian): Hooks['tool.execute.before'] {
  return async (input, output) => {
    const { tool, args, sessionID } = input as { tool: string; args?: Record<string, unknown>; sessionID?: string };
    
    // L0: IDENTITY WALL — Only process for shark agent sessions
    const currentAgent = getCurrentAgent(sessionID);
    if (!currentAgent) {
      console.log(`[SHARK DEBUG] getCurrentAgent(${sessionID}) returned undefined. Tool: ${tool}`);
      return;
    }

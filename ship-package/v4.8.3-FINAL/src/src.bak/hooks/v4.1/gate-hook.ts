 ...
 * 
 * V4.7: Also tracks edit history for source file protection.
 * 
...
export function createGateHook(
  ...
  return async (input, output) => {
    ...
    // V4.7: Track edit history for source file protection
    if (guardian && args) {
      const editArgs = args as { filePath?: string };
      
      // Track edit tool
      if (tool === 'edit' && editArgs?.filePath) {
        const filePath = editArgs.filePath;
        const success = !result || !JSON.stringify(result).includes('"error"');
        if (success) {
          guardian.registerEdit(filePath);
        }
      }
      
      // Track write tool (creates new files)
      if ((tool === 'write_file' || tool === 'mcp_write_file') && editArgs?.filePath) {
        const filePath = editArgs.filePath;
        const success = !result || !JSON.stringify(result).includes('"error"');
        if (success) {
          guardian.registerCreate(filePath);
        }
      }
    }

...
export function createChatMessageHook(): Hooks['chat.message'] {
  return async (input, output) => {
    ...
    if (agentName && isSharkAgent(agentName)) {
      ...
      // V4.8.3 CP4.2: CRITICAL FIX — chat.message fires BEFORE agent responds.
      // output.message.parts IS the user message, NOT agent output.
      // All anti-derailment checking happens in messages.transform hook,
      // which fires AFTER agent responds and has proper role separation.
      // This hook ONLY initializes brain state. Period.
    }
  };
}

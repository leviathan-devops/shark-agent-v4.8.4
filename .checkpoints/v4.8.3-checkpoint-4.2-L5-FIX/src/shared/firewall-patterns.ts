...
export const SOURCE_INSPECTION_PATTERNS = [
  /test\s+-[fe]\s+/i, /test\s+-d\s+/i,
  /if\s+\[\s*-[fes]\s+.*\]\s*;/i, /\[\s*-[fe]\s+.*\]/i,
  /grep\s+-r\s+.*src\//i, /ls\s+-l.*(dist|src|build)\//i,
  /stat\s+/i, /find\s+.*src/i,
];
...
export const IMPATIENCE_PATTERNS = [
  ...
  /just.*ship.*it/i, /good.*enough/i, /close.*enough/i,
